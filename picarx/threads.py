...
if __name__ == "__main__":
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        for i in range(0, 3):
            match i:
                case 0: 
                    eSensor = executor.submit(sensor_function, sensor_values_bus, 0.25)
                    eSensor.add_done_callback(handle_exception)
                case 1:
                    eInterpreter = executor.submit(interpreter_function,sensor_values_bus, interpreter_bus,0.25)
                    eInterpreter.add_done_callback(handle_exception)
                case 2:
                    eController = executor.submit(controller_function, interpreter_bus, 0.5)
